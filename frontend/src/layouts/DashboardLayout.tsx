...
export function DashboardLayout() {
  ...
  return (
    <div className="bg-background text-on-background min-h-screen flex font-body-md text-body-md antialiased selection:bg-primary-container selection:text-on-primary-container">
      {/* SideNavBar */}
      <nav className="hidden md:flex flex-col fixed left-0 top-0 h-screen w-64 bg-surface dark:bg-inverse-surface z-40">
        <div className="h-16 bg-[#000000] border-b border-outline-variant flex items-center justify-center shrink-0">
          <img src="/logo.png" alt="Eco Plug Logo" className="max-h-12 w-auto object-contain" />
        </div>
        <div className="flex-1 flex flex-col p-md overflow-y-auto border-r border-outline-variant dark:border-outline">
      ...
      <div className="flex-1 flex flex-col md:ml-64 h-screen overflow-hidden pt-16">
        {/* TopNavBar */}
        <header className="hidden md:flex fixed top-0 right-0 w-[calc(100%-16rem)] h-16 bg-[#000000] border-b border-outline-variant dark:border-outline justify-end items-center px-lg z-30 transition-all duration-200">
          <div className="flex items-center gap-md">
            <div className="flex items-center gap-3 cursor-pointer hover:bg-white/10 px-3 py-1.5 rounded-full transition-colors">
              ...
              <span className="font-label-md text-label-md text-white font-medium">
                {user?.full_name || 'User'}
              </span>
